...
Identical machinery to edu_clean's Approach B (the algorithm is field-agnostic);
here it runs on company names and job titles. Pipeline per field:
  ...
  3. within each block, union-find merge pairs that pass a *guarded* predicate:
     ...
     that stops subset/superset false-merges ("Software Engineer" vs "Senior
     Software Engineer", "Citizens Bank" vs "First Citizens Bank").
  4. canonical = highest-frequency member of each cluster.

This buys typo / spacing tolerance that pure normalization (Approach A) lacks,
while staying precise about distinct roles and organizations. It cannot expand
abbreviations (VP -> Vice President, EY -> Ernst & Young) -- that is Approach A's
job via rules / the company_id key.
"""
