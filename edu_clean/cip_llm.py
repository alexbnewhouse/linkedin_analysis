"""LLM jury for CIP 2-digit family coding of the uncoded field-of-study tail.

Mirrors ``career_clean/soc_llm.py`` (which mirrors the industry package's
local-jury layer) but targets the 41 CIP 2020 families observed in the data
(``edu_clean/cip_taxonomy.py``). The generic host pool
(``industry/llm_pool.py``) is imported VERBATIM.

Honesty rules baked in here:
  * ``evidence_text`` renders ONLY fields computed by ``cip_candidates.py``
    from the string's own education rows -- absent context is omitted, never
    invented. (The modal degree text is truncated to 80 chars at render time;
    a truncation, not a fabrication.)
  ...
  * Votes are propose-only: they land in an append-only JSONL cache keyed by
    ...
    until ``run_cip_jury merge``, and even that writes a NEW mappings parquet.
"""
...
CACHE_FILE = Path(__file__).resolve().parent / "results" / "cip_votes.jsonl"

# The two-juror unanimity panel: disjoint sizes/architectures on separate
# llama-server lanes. Acceptance = both emit the SAME non-XUN family
...
# Copied verbatim from career_clean.soc_llm.HOSTS_ENV.
HOSTS_ENV = (
...
# thinking output). bulk-moe-q4 is Qwen3-30B-A3B, hence listed.
_THINK_OFF_PREFIXES = ("qwen3", "bulk-moe", "deepseek-r1")

NUM_CTX = 6144  # per-slot ctx on every lane; prompt must stay well under this

_DEGREE_TRUNC = 80  # render-time cap on the modal degree text


def _catalog() -> str:
    return "\n".join(f"  {c} - {T.FAMILIES[c]}" for c in sorted(T.FAMILIES))


_SYSTEM = f"""You classify ONE field of study into exactly one CIP 2020 2-digit family.
...
Rules:
1. Judge the FIELD named by the string, not the career it might lead to: "nursing" is Health Professions (51) even though nurses work in hospitals.
2. Foreign-language field names are common. Classify them by meaning when unambiguous ("informatica" = computer science -> 11, "derecho" = law -> 22); if the meaning is ambiguous, answer {T.ABSTAIN}.
3. Joint or double majors ("English and History", "Finance and Marketing") are {T.ABSTAIN} unless one field clearly dominates the string.
4. Strings that name NO field of study are {T.ABSTAIN}: institution names, degree types alone ("MBA", "Associate's"), grades or honors ("3.8", "cum laude"), and non-answers ("N/A"). But "General Studies" IS a real program family -> 24 Liberal Arts and Sciences, General Studies and Humanities.
5. Follow the reference conventions demonstrated by the "Nearest reference fields" line: those are real field strings from this dataset with the families the official mapping assigned them. When your own reading differs from a clear convention shown by close reference fields, follow the reference convention.
6. If the field could belong to two or more different families and neither the references nor the evidence settle it, answer {T.ABSTAIN}. Never guess between plausible families.
7. Answer in the fixed JSON schema: first a rationale of at most two short sentences, then the code, then your confidence.
"""


def evidence_text(item: dict) -> str:
    """Render ONLY provided evidence; omit absent fields entirely."""
    parts = [f"Field of study: {item.get('field_norm') or '(unknown)'}"]
    if item.get("top_raw_variants"):
        parts.append(f"As written on profiles: {item['top_raw_variants']}")
    if item.get("modal_degree_text"):
        parts.append(f"Typical degree line: {item['modal_degree_text'][:_DEGREE_TRUNC]}")
    if item.get("n_rows"):
        parts.append(f"Occurrences in the dataset: {item['n_rows']} education rows.")
    if item.get("anchors"):
        parts.append("Nearest reference fields (already-coded strings from this dataset, exact matches excluded):\n"
...
class Proposal:
    key: str            # field_norm
    code: str           # CIP family or XUN (validated before caching)
...
def load_cache() -> dict[str, Proposal]:
    ...
    for line in CACHE_FILE.read_text().splitlines():
        ...
        out[p.input_hash] = p
    if bad:
        print(f"[cip_llm] WARNING: skipped {bad} malformed cache line(s)")
    return out
...
def votes_by_key(items: list[dict], jurors: tuple[str, ...] = JURY) -> dict[str, dict[str, str]]:
    """{field_norm: {model: code}} for whatever is already cached."""
    cache = load_cache()
...
def fire(items: list[dict], jurors: tuple[str, ...] = JURY, *, execute: bool = False) -> dict:
    ...
    pool = make_pool()
    if not pool.hosts:
        print("[cip_llm] no llamacpp lane reachable -- nothing fired")
        return {**plan, "done": 0, "failed": 0, "skipped": len(pending)}
    print(f"[cip_llm] firing {len(pending):,} units across "
    ...
    if buf:
        append_cache(buf)
    print(f"[cip_llm] pool: {stats['done']:,} ok, {stats['failed']:,} failed, "
