...
| `onet_skills_15_1252_imputed.txt` | **derived**: O\*NET 22.0 `Skills.txt` Scale ID='IM' rows for 15-1132.00 and 15-1133.00 only (70 rows) | O\*NET-SOC **22.0** (source rows), imputing a **29.3** skills-table gap | [onetcenter.org/database](https://www.onetcenter.org/database.html) (Skills, db_22_0_text release) | CC BY 4.0 (O\*NET) | `skill_breadth/onet_skills.py` (imputes SOC 15-1252 "Software Developers", which has no `Skills.txt` rows in any O*NET release 25.1-29.3, as the mean of these two O*NET 22.0 pre-split detail codes, joined to 29.3 skill names by Element ID; see the file's own header comment and `skill_breadth/onet_skills.py`'s module docstring) |
