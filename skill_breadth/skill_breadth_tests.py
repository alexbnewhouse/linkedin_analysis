...
def main() -> None:
    ...
    got_persons = con.sql('SELECT linkedin_id, "group", bachelor_end_year, bachelor_cip '
                           'FROM fx_persons ORDER BY linkedin_id').fetchall()

    check("one person per headline group, multi-group and out-of-window excluded",
          got_persons == [
